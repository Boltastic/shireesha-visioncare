...
export default function AdminLogin() {
  ...
  const login = trpc.adminAuth.login.useMutation({ onSuccess: () => { utils.adminAuth.session.invalidate(); notify({ kind: "success", title: "Staff access granted", message: "Welcome to the Vision Care dashboard." }); }, onError: error => notify({ kind: "error", title: "Sign-in unsuccessful", message: error.message }) });
